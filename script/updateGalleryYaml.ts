...
imageGroups.forEach((group) => {
  ...
  let updatedArchive =
    (archiveYamlData.mtime || 0) !== manageData[group].archive.mtime;
  ...
  if (updated) {
    ...
    manageData[group].mtime = Number(
      Deno.statSync(yamlData.path).mtime?.getTime(),
    );
  }
  if (updatedArchive) {
    ...
    manageData[group].archive.mtime = Number(
      Deno.statSync(archiveYamlData.path).mtime?.getTime(),
    );
  }
