...
function settingElm(elm = document) {
  elm.querySelectorAll(`[data-setting-cookie-enable]`).forEach((e) => {
    e.innerHTML =
      `<input type="checkbox" onclick="$COOKIE.outEnable=!$COOKIE.outEnable; this.checked = $COOKIE.outEnable;" ${
        $COOKIE.outEnable ? "checked" : ""
      } />`;
  });
  elm.querySelectorAll(`[data-setting-analytics-ignore]`).forEach((e) => {
    e.innerHTML =
      `<input type="checkbox" onclick="$COOKIE.toggle({key:'analytics-ignore'}); this.checked = toBoolean($COOKIE['analytics-ignore']);" ${
