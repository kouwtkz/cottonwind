        ...
        transitions: [
            {
                ...
                // 主にページセクションを調べ、現在のページセクションを変更する部分
                leave(data) {
                    ...
                    if (!tweet_iframe_elm) {
                        const tweet_iframe =
                            data.current.container.querySelector(
                                `iframe[src="/inline/twitter_kouwtkz.html"]`
                            );
                        if (tweet_iframe) {
                            const ibody = tweet_iframe.contentDocument.body;
                            if (!ibody.querySelector("a.twitter-timeline")) {
                                tweet_iframe_elm = ibody.firstChild;
                            }
                        }
                    }
                },
                ...
                beforeEnter(data) {
                    ...
                    // ツイートフレーム取得済みの場合は入れ替える
                    if (tweet_iframe_elm) {
                        const tweet_iframe = data.next.container.querySelector(
                            `iframe[src="/inline/twitter_kouwtkz.html"]`
                        );
                        if (tweet_iframe) {
                            tweet_iframe.parentElement.append(tweet_iframe_elm);
                            tweet_iframe.parentElement.removeChild(
                                tweet_iframe
                            );
                        }
                    }
